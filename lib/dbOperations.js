...
async function getUserSearch({ data, userModel, fellowshipModel }) {
  ...
  if (strangersData.length > 0) {
    ...
    unAddableStrangers = await fellowshipModel.findAll({
      where:
      {
        [and]: [
...
async function getUserById({data, model}) {
  const user = await model.findOne({
    where:
    {
      id: data.id
...
async function getAllPublishers({ data, fellowshipModel, postModel }) {
  ...
  // Filter out posts the viewer may be permitted to see but may not want to
  const finalPosts = {};
  finalPosts.rows = [];
  for (const post of posts.rows) { 
    ...
    if (post.userId != data.id) {
      for (const tag of post.tags) {
        if (!data.viewTags.includes(tag) && tag != data.publicTag) {
          addable = false;
        }
      }
      if (addable == true) {
        finalPosts.rows.push(post);
      }
    }
    else {
      finalPosts.rows.push(post);
...
async function expandPostInfo({ data, userModel, tagModel }) {
  for (const post of data.rows) {
    ...
    post.tags = post.tags.map((tag) => { return tag.name });
  }
  return data;
}

async function getNewestPublicPost({ data, postModel, tagModel }) {
  const rangeConstraints = {postLimit: 10, postOffset: 0};
  rangeConstraints.postLimit = data.postLimit ? data.postLimit : process.env.PG_POST_LIMIT;
  rangeConstraints.postOffset = data.postOffset ? data.postOffset : process.env.PG_POST_OFFSET;
  ...
  for (i = 0; i < data.targets.length; i++){
    data.targets[i].posts = await postModel.findAll({
      where: {
        [and]: [
  ...
  for (const target of data.targets) {
    ...
    if (target.finalPosts.length > 1) {
      target.finalPosts = [target.finalPosts[0]];
    }
  }
  for (const target of data.targets) {
    target.posts = target.finalPosts; 
  }
  for (i = 0; i < data.targets.length; i++){
    for (h = 0; h < data.targets[i].posts.length; h++){
      for (j = 0; j < data.targets[i].posts[h].tags.length; j++) {
        for (k = 0; k < publicTagRecord.length; k++) {
          if (data.targets[i].posts[h].tags[j] == publicTagRecord[k].id){
            data.targets[i].posts[h].tags[j] = publicTagRecord[k].name;
          }
        }
      }
    }
...
async function getUserPublishers({ data, model }) {
  const publishers = await model.findAll({
    where:
    {
      subscriberId: data.id
    }
...
async function getUserSubscribers({ data, model }) {
...
async function downloadData({ data, models }) {
  const tags = await getTags({ model: models.tagModel });
  const tagsDict = {};
  for (const tag of tags) {
    tagsDict[tag.id] = tag.name;
  }

  const user = await getUser({ data, model: models.userModel });
  userContent = `${user.username},${user.password},${user.email},${user.dateTimeCreated}`;
  userContent += '\n';

  const userPosts = await getPosts({ data: { id: user.id }, model: models.postModel });
  ...
  userPostsContent = ``;
  for (const post of userPosts.rows) {
    userPostsContent += `${post.tags},${post.content},${post.dateTimeCreated}\n`;
  }
  userPostsContent += '\n';
  
  const userPublishers = await getUserPublishers({ data: { id: user.id }, model: models.fellowshipModel });
  const userPublishersNames = [];
  for (i = 0; i < userPublishers.length; i++) {
    userPublisher = await getUserById({ data: { id: userPublishers[i].publisherId }, model: models.userModel });
    userPublishersNames.push(userPublisher.username);
  }
  userPublishersContent = ``;
  for (i = 0; i < userPublishersNames.length; i++) {
    userPublishersContent += `${userPublishersNames[i]}`;
    if (i + 1 < userPublishersNames.length) {
      userPublishersContent += ',';
    }
  }
  userPublishersContent += '\n';

  const userSubscribers = await getUserSubscribers({ data: { id: user.id }, model: models.fellowshipModel });
  const userSubscribersNames = [];
  for (i = 0; i < userSubscribers.length; i++) {
    userSubscriber = await getUserById({ data: { id: userSubscribers[i].subscriberId }, model: models.userModel });
    userSubscribersNames.push(userSubscriber.username);
  }
  userSubscribersContent = ``;
  for (i = 0; i < userSubscribersNames.length; i++) {
    userSubscribersContent += `${userSubscribersNames[i]}`;
    if (i + 1 < userSubscribersNames.length) {
      userSubscribersContent += ',';
    }
  }
  userSubscribersContent += '\n';

  const content = userContent+userPublishersContent+userSubscribersContent+userPostsContent;
  fs.writeFile(`${user.username}.csv`, content, function (err) {
    if (err) return console.log(err);
    // console.log(`${content} > ${user.username}.csv`);
  });
  const fileInfo = { fileName: `${user.username}.csv`, fileContents: content };
  return fileInfo;
}
